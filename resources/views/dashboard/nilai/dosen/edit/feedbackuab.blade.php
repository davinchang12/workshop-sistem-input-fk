...
@section('container')
    ...
    <form action="/dashboard/matkul/nilai/edit/feedbackuab/simpan" method="post">
        @csrf
        ...
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h3 class="h5">Edit Nilai Feedback UAB</h3>
        </div>
        ...
        <table class="table" style="text-align: center">
            ...
            <tbody>
                @foreach ($uabs as $uab)
                    <tr>
                        <input type="hidden" name="jenisfeedback[]" id="jenisfeedback[]" value="{{ $uab->jenisfeedback_id }}">
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $uab->name }}</td>
                        <td>
                            {{ $uab->nim }}
                            <input type="hidden" name="nim[]" id="nim[]" value="{{ $uab->nim }}">
                        </td>
                        <td>
                            <input type="number" max="100" min="0" step="0.01"
                                style="border: none; font-size:18px; width:100%; text-align: center;" name="skor[]"
                                id="skor[]" value="{{ $uab->skor }}">
                        </td>
                        <td>
                            {{ $uab->topik }}
                            <input type="hidden" name="topik[]" id="topik[]" value="{{ $uab->topik }}">
                        </td>
                    </tr>
                @endforeach
            </tbody>
